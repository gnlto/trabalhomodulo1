function submit(){
    ...
    alert("Tenha atenção ao fazer suas escolhas! O futuro da raça humana depende exclusivamente de você!");
    ...
    while (personagem != "ANAKAN" && personagem != "LAIKA" && personagem != "COWBOY ESPACIAL" || personagem ===""){
        alert("Atenção! Escolha um personagem disponível.")
        var personagem = prompt("Escolha um aventureiro para percorrer a jornada e garantir o futuro da humanidade! Anakan, Laika ou Cowboy Espacial.");
        var personagem = personagem.toUpperCase();
    }
    ...
    if (personagem === 'ANAKAN'){
    ...
    } else if (personagem === "COWBOY ESPACIAL"){
        alert("Cowboy Espacial chega ao seu destino, o planeta Rex-18. Enquanto sobrevoa a área tentando rastrear RES-ILIA, sua nave é atacada por piratas espaciais.");
        alert("Após o ataque, Cowboy identifica pelo painel de controle que a nave teve a estrutura danificada.");

        var cowboyEscolha = prompt("A - Abandonar a nave em um Pod; \nB - Tentar realizar um pouso de emergência.");
        ...
        if (cowboyEscolha === "A"){
            alert("O Pod é ejetado enquanto a nave vai em direção ao solo. Chegando ao chão, Cowboy Espacial nota que a nave dos piratas está a caminho dos destroços de sua nave.");
            alert("Ele começa a caminhar nas dunas com o pouco equipamento que tem e avista um buraco enorme com várias árvores ao redor, indicando a existência de água e possivelmente do elemento RES.");
            alert("Ao se aproximar, Cowboy percebe que para descer pode ir por um caminho no qual ele não enxerga nada ou realizar um salto de fé para tentar cair no que parece ser um lago fundo.");

            var cowboyEscolha2 = prompt("A - Tentar o salto de fé; \nB - Percorrer o caminho escuro.");
            ...
            if (cowboyEscolha2 === "A"){
                alert("Ao realizar o salto de fé, Cowboy Espacial se vê em um lago de profundidade desconhecida, porém identifica o que parece ser um material brilhante abaixo dele.");
                alert("Ao seu redor, apenas uma estrutura de pedras em ruínas e árvores, sem identificação de margem para exploração.");

                var cowboyEscolha3 = prompt("A - Cowboy Espacial deve mergulhar e explorar o possível objeto brilhante; \nB - Cowboy Espacial deve persistir e explorar as ruínas.");
                ...
                if (cowboyEscolha3 === "A"){
                    ...
                    alert("No momento em que sai do lago, Cowboy escuta um barulho estridente vindo dos cantos da ruína, mas mesmo assim decide continuar a exploração.");
                    alert("Sem tempo de reação, Cowboy Espacial é arremessado por uma criatura e não sobrevive.");
                    alert("GAME OVER! Tente novamente.");
                }

            } else if (cowboyEscolha2 === "B"){
                alert("Cowboy tenta seguir o caminho nas dunas escuras e encontra uma criatura colossal. Sem suas armas, que ficaram na nave, o nosso viajante é atacado e morre.");
                alert("GAME OVER! Tente novamente.");
            }

        } else if (cowboyEscolha === "B"){
            alert("Mesmo com a estrutura de sua nave comprometida, ele consegue aterrizar, mas os piratas, que vieram roubar os componentes de sua nave, são muitos...");
            alert("GAME OVER! Tente novamente.");
        }

    } else if (personagem === "LAIKA"){
        alert("Laika vai para um planeta de gelo, o Midnight-Mistery01, que já havia sido habitado por uma civilização, mas a temperatura tornou a possibilidade de vida mais improvável.");
        alert("Acredita-se que o artefato conhecido como Golden Circle pode estar soterrado em alguma parte da superfice congelada.");
        alert("Ao chegar no planeta com sua nave, Laika identifica, através de seus equipamentos, uma estrutura subterrânea.");

        var laikaEscolha = prompt("A - Laika decide adentrar a estrutura subterrânea; \nB - Laika decide continuar sobrevoando o planeta.");
        ...
        if (laikaEscolha === "A"){
            alert("Chegando na estrutura subterrânea, Laika identifica uma caverna. Após caminhar por algum tempo dentro dela, a viajante encontra uma bifurcação.");
            alert("Laika, porém, sabe que seu tempo é curto e precisa de uma decisão rápida.");

            var laikaEscolha2 = prompt("A - Percorrer o caminho da esquerda; \nB - Percorrer o caminho da direita.");
            ...
            if (laikaEscolha2 === "B"){
                alert("Laika segue o caminho da direita e encontra uma torre gigante feita de gelo. Ela usa seus equipamentos para escalar até a entrada e se depara com uma mesa e um item sobre ela.");
                alert("Laika usa seu identificador de objetos e tem certeza de que se trata do Golden Circle. O que Laika deve fazer?");

                var laikaEscolha3 = prompt("A - Laika deve avançar até a mesa e coletar o artefato; \nB - Laika inspeciona o ambiente antes de avançar em direção ao artefato.");
                ...
                if (laikaEscolha3 === "B"){
                    ...
                    alert("No momento em que Laika avança para atravessar a sala, uma armadilha é ativada, o que coloca um fim na jornada de Laika.");
                    alert("GAME OVER! Tente novamente.");
                }
            ...
            alert("Laika segue sobrevoando o planeta, mas é surpreendida por uma tempestade de gelo que danifica o sistema de elerólise de sua nave, a deixando sem oxigênio.");
            alert("GAME OVER! Tente novamente.");
        }// else if para os outros personagens, seguindo a mesma base do código do personagem anakan
    }
}